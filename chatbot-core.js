class Chatbot {
    ...
    handleSendMessage(text) {
        ...
        if (this.currentTypingIndicator) {
             // Rimuovi il precedente se per qualche motivo esiste ancora
             console.warn("Rimosso indicatore di scrittura precedente non eliminato.");
    ...
    async sendMessageToApi(messageText) {
        console.log("Chatbot Core: Invio messaggio all'API:", messageText);
        ...
        if (!apiToken) {
            console.error("Chatbot Core: Token API non disponibile. Impossibile inviare messaggio.");
            // Gestire errore UI (Task 5)
        ...
        try {
            ...
            if (response.ok) {
                try {
                    ...
                    if (Array.isArray(data) && data.length > 0 && data[0].text) {
                        ...
                        console.log("Chatbot Core: Risposta API estratta:", botReplyText);
                        // Potremmo usare data[0].user e data[0].action in futuro
                    } else {
                        console.warn("Chatbot Core: Struttura risposta API non riconosciuta o testo mancante.", data);
                        botReplyText = "Ho ricevuto una risposta ma non sono riuscito a leggerla correttamente.";
                    }
                    // --- FINE NUOVA ESTRAZIONE ---
                    
                } catch (jsonError) {
                    console.error("Chatbot Core: Errore parsing JSON dalla risposta API:", jsonError);
                    errorMessage = "Errore nella lettura della risposta del bot.";
                }
            } else {
                console.error(`Chatbot Core: Errore API - Status: ${response.status} ${response.statusText}`);
            ...
            if (this.currentTypingIndicator) {
                ...
                this.currentTypingIndicator = null; // Imposta a null dopo rimozione
            }
            ...
            if (errorMessage) {
                ...
                ChatbotUI.displayMessage(this.elements.messageArea, botReplyText, 'bot');
                 ...
                 // Caso strano: nessuna risposta e nessun errore? Loggalo.
                 console.warn("Chatbot Core: Nessuna risposta o errore da mostrare.");
                 // Potresti mostrare un messaggio generico qui se necessario
            }

        } catch (error) {
            console.error("Chatbot Core: Errore durante la chiamata fetch all'API:", error);
